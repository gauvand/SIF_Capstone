...
def filter_standardization(raw,window_size = 1000,
    ...
    filtered_standardized = ((filtered_X - filtered_X.rolling(window_size).mean()) / \
                             filtered_X.rolling(window_size).std()).dropna()
    ...
    # get data for target signal
    data = raw.get_data()
    
    if read_event:
        # padding now
        target_signal = target_signal_val =data[32:38,:] # export all channels, use only one channel eventually
...
def train_model(model, ttype, train_loader, val_loader, 
                optimizer, loss_func, epochs, loss_buffer_size=4, 
                print_freq = 10, prog_bar=None):
    ...
    for e in progress_bar:
        ...
        e_time = end - start
        loss_track['epoch_time'].append(e_time)
...
def load_data(train_x_t ,train_y_t,
              val_x_t ,val_y_t,
              batch_size = 64):
    # batch_size is a hyper parameter to tune 
    train_dataset = EEGDataset(train_x_t ,train_y_t)
